...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
		...
		maxpa = (uint32_t)nextfree + npages * PGSIZE; 
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pde & PTE_P) {
		...
		pte_pt = (pte_t *)(KADDR(PTE_ADDR(pde)));  // page table address
...
{
	pte_t* pte_pt = pgdir_walk(pgdir, va, false);
	if (pte_store) {
		*pte_store = pte_pt;
	}
	if (pte_pt) {
		return pa2page(PTE_ADDR(*pte_pt));
	}else {
		return NULL;
	}
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t va_s = (uintptr_t)va;
	uintptr_t va_e = (uintptr_t)ROUNDUP((uintptr_t)va + len, PGSIZE);
	
	for (; va_s < va_e; va_s = ROUNDDOWN(va_s + PGSIZE, PGSIZE)) {
		pte = pgdir_walk(env->env_pgdir, (void *)va_s, 0);
		if ((va_s >= ULIM) || !pte || ((*pte & perm) != perm) || (!(*pte & PTE_P))) {
			user_mem_check_addr = va_s;
			return -E_FAULT;
		}
	}
